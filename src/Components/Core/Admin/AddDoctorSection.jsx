...
export const AddDoctorSection = () => {
  ...
  // Form State (No ID field needed here, backend handles it)
  ...
  // Temporary state for adding a single qualification
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctors.filter(doc => 
      ...
      // Search by the backend-generated doctorID (e.g., 1001)
      (doc.doctorID && doc.doctorID.toString().includes(q)) 
    );
  }, [doctors, searchQuery]);
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
                <CardTitle className="flex items-center gap-2">
                    {isEditing ? <Edit className="w-5 h-5 text-indigo-600"/> : <Stethoscope className="w-5 h-5 text-indigo-600"/>} 
                    {isEditing ? "Update Profile" : "Onboard Doctor"}
                </CardTitle>
                {isEditing && <Button variant="ghost" size="sm" onClick={resetForm} className="text-red-500"><X className="w-4 h-4 mr-1"/> Cancel</Button>}
            </div>
            <CardDescription>
                {isEditing ? "Editing details for ID: " + editId : "Enter details below. Doctor ID (1001+) is generated automatically."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <Select onValueChange={(v) => handleSelectChange("department", v)} value={formData.department}>
                    ...
                    <SelectContent>
                      {["Cardiology", "Neurology", "Pediatrics", "Orthopedics", "Dermatology", "General Surgery", "Oncology", "Psychiatry"].map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1"><Label>Specialization</Label><Input id="specialization" placeholder="e.g. Spine Surgeon" value={formData.specialization} onChange={handleInputChange} /></div>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><Label>Experience</Label><Input id="experience" placeholder="e.g. 10 Years" value={formData.experience} onChange={handleInputChange} required /></div>
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-2">
                  ...
                  <div className="flex gap-2">
                      <Input placeholder="Degree (e.g. MBBS)" value={newQual.degree} onChange={e=>setNewQual({...newQual, degree: e.target.value})} className="bg-white"/>
                  ...
                  <div className="space-y-1">
                      {formData.qualifications.map((q, i) => (
                          <div key={i} className="flex justify-between items-center text-sm bg-white p-2 rounded border shadow-sm">
                            <span className="text-slate-700 font-medium">{q.degree} <span className="text-slate-400 font-normal">from {q.college}</span></span>
                            <button type="button" onClick={()=>removeQual(i)} className="text-red-500 hover:bg-red-50 p-1 rounded"><X className="w-3 h-3"/></button>
                          </div>
                      ))}
                  </div>
              </div>
              ...
              <Button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700" disabled={isLoading}>
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin"/> : isEditing ? "Save Changes" : "Create Doctor Account"}
              </Button>
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="h-[850px] flex flex-col border-slate-200 shadow-sm">
          <CardHeader>
              ...
              <div className="relative mt-2">
                  ...
                  <Input 
                    placeholder="Search by ID (e.g. 1001), Name, Dept..." 
                    className="pl-9" 
                    value={searchQuery} 
          ...
          <CardContent className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {isFetching ? (
                ...
                <div className="text-center py-10 text-slate-400">No doctors found.</div>
            ) : (
                <div className="space-y-3">
                    {filteredDoctors.map(doc => (
                        <div key={doc._id} className="flex justify-between items-start p-4 border border-slate-100 rounded-lg hover:bg-slate-50 transition-colors bg-white shadow-sm">
                            <div className="flex gap-4">
                                {/* Initials Avatar */}
                                <div className="h-10 w-10 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center font-bold text-sm border border-indigo-100 shrink-0">
                                ...
                                <div>
                                    <div className="flex flex-wrap items-center gap-2 mb-1">
                                        ...
                                        {/* Display Backend Generated ID */}
                                        {doc.doctorID && (
                                            <span className="flex items-center gap-1 text-[10px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500 font-mono border border-slate-200">
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-2">
                                        {doc.qualification?.slice(0, 3).map((q, i) => (
                                            <span key={i} className="text-[10px] bg-slate-50 px-1.5 py-0.5 rounded text-slate-600 border border-slate-200">
                                                {q.degree}
                                            </span>
                                        ))}
                                        {doc.qualification?.length > 3 && <span className="text-[10px] text-slate-400">+{doc.qualification.length - 3} more</span>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-1 shrink-0">
                                <Button variant="ghost" size="icon" onClick={()=>handleEditClick(doc)} className="text-blue-600 h-8 w-8 hover:bg-blue-50" title="Edit">
                                    <Edit className="w-4 h-4"/>
                                </Button>
                                <Button variant="ghost" size="icon" onClick={()=>handleDelete(doc._id)} className="text-red-500 h-8 w-8 hover:bg-red-50" title="Delete">
