...
import { 
  ...
  FileText, Activity, Pill, Plus, Trash2, Eye, Edit3, 
  Search, AlertCircle, Paperclip, Download, ExternalLink, X 
} from "lucide-react";
...
export default function AppointmentSection() {
  ...
  const [existingReportData, setExistingReportData] = useState(null); // For Editing
  const [viewReportData, setViewReportData] = useState(null); // For Viewing History
  ...
  // --- 2. Status Handler (Confirm / Cancel) ---
  const handleStatusChange = async (id, newStatus) => {
    try {
      // Backend handles freeing the timeslot if status === "Cancelled"
  ...
  // Open "Edit Report" Modal (from History)
  const handleEditReport = async (appt) => {
      ...
      if (data) {
          ...
          toast({ title: "Error", description: "Could not fetch existing report" });
      }
  };

  // Called after report is successfully submitted
  const handleReportSuccess = (appointmentId) => {
    // Backend marks appointment as "Completed" automatically on report creation
    // We update local state to move it to History tab immediately
    setAppointments(prev => prev.map(a => a._id === appointmentId ? { ...a, status: "Completed" } : a));
    setIsReportOpen(false);
    toast({ title: "Encounter Completed", description: "Report generated and sent to patient." });
  };
  ...
  // --- 4. Filtering Logic (Active vs History) ---
  ...
  // ACTIVE QUEUE: All appointments EXCEPT those that are 'Completed' or 'Cancelled'
  // This keeps 'Pending', 'Scheduled', and 'Confirmed' in the active view regardless of date.
  const activeQueue = filteredAppointments.filter(a => 
      !["Completed", "Cancelled"].includes(a.status)
  );

  // HISTORY: Only 'Completed' (Report Generated) or 'Cancelled'
  const historyQueue = filteredAppointments.filter(a => 
      ["Completed", "Cancelled"].includes(a.status)
  );
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="queue" className="w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="relative w-full sm:w-64">
                ...
                <Input placeholder="Search patient..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="pl-10 h-9 bg-white" />
            </div>
        </div>

        {/* --- ACTIVE QUEUE TAB --- */}
        <TabsContent value="queue" className="space-y-4">
            {loading ? <div className="text-center py-10 text-slate-400">Loading queue...</div> : activeQueue.length === 0 ? (
        ...
        <TabsContent value="history" className="space-y-4">
            {loading ? <div className="text-center py-10 text-slate-400">Loading history...</div> : historyQueue.length === 0 ? (
        ...
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 gap-0">
          <DialogHeader className="p-5 border-b bg-slate-50 flex-shrink-0">
            <DialogTitle className="flex items-center gap-2 text-slate-800">
              <Activity className="h-5 w-5 text-blue-600" /> {isEditMode ? "Edit Clinical Report" : "New Clinical Encounter"}
            </DialogTitle>
          </DialogHeader>
          {selectedAppt && (
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-3xl h-[90vh] flex flex-col p-0">
          <DialogHeader className="p-5 border-b flex-shrink-0"><DialogTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-green-600" /> Clinical Record</DialogTitle></DialogHeader>
                ...
                <div className="flex flex-col items-end gap-3 min-w-[160px]">
                    ...
                    {!isHistory ? (
                        <div className="flex gap-2">
                            {/* ACTION: Confirm/Cancel (For Pending/Scheduled) */}
                            {(appt.status === "Scheduled" || appt.status === "Pending") && (
                                <>
                                    <Button size="sm" variant="outline" className="h-8 text-emerald-600 border-emerald-200 hover:bg-emerald-50" onClick={() => onStatusChange(appt._id, "Confirmed")}>
                                        <CheckCircle2 className="w-3 h-3 mr-1"/> Confirm
                                    </Button>
                                    <Button size="sm" variant="ghost" className="h-8 text-red-500 hover:bg-red-50 hover:text-red-700" onClick={() => onStatusChange(appt._id, "Cancelled")}>
                                        <XCircle className="w-3 h-3 mr-1"/> Reject
                                    </Button>
                                </>
                            )}

                            {/* ACTION: Initiate (Only if Confirmed) */}
...
// ============================================================================
// SUB-COMPONENT: WRITE REPORT FORM
// ============================================================================
function CreateDetailedReportForm({ appointment, token, onSuccess, onCancel, isEditMode, initialData }) {
  ...
  const removeExistingFile = (fileUrl) => { setExistingLabReports(prev => prev.filter(f => f.url !== fileUrl)); setDeletedLabReports(prev => [...prev, fileUrl]); };

  const handleSubmit = async () => {
    if (!formData.diagnosis) { toast({ title: "Required", description: "Primary Diagnosis is mandatory.", variant: "destructive" }); return; }
    setLoading(true);
    ...
    try {
        if (isEditMode) { await updateVisitReport(token, data); toast({ title: "Updated", description: "Clinical record updated." }); } 
        else { await createVisitReport(token, data); toast({ title: "Generated", description: "Encounter report created." }); }
        onSuccess();
        ...
        <ScrollArea className="flex-1 p-6">
            <div className="space-y-8 pb-10">
                ...
                <div className="space-y-3">
                    <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider flex items-center gap-2"><Activity className="w-4 h-4"/> Vitals & Anthropometry</h3>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div className="space-y-1"><Label className="text-xs text-slate-500">BP (mmHg)</Label><Input name="bp" placeholder="120/80" value={formData.bp} onChange={handleChange} className="h-9"/></div>
                        <div className="space-y-1"><Label className="text-xs text-slate-500">Weight (kg)</Label><Input name="weight" placeholder="70" value={formData.weight} onChange={handleChange} className="h-9"/></div>
                        <div className="space-y-1"><Label className="text-xs text-slate-500">Temp (°F)</Label><Input name="temperature" placeholder="98.6" value={formData.temperature} onChange={handleChange} className="h-9"/></div>
                        <div className="space-y-1"><Label className="text-xs text-slate-500">SpO2 (%)</Label><Input name="spo2" placeholder="98" value={formData.spo2} onChange={handleChange} className="h-9"/></div>
                        <div className="space-y-1"><Label className="text-xs text-slate-500">Pulse (bpm)</Label><Input name="heartRate" placeholder="72" value={formData.heartRate} onChange={handleChange} className="h-9"/></div>
                    </div>
                </div>
                <Separator />
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center">
                        <h3 className="text-xs font-bold uppercase text-slate-400 tracking-wider flex items-center gap-2"><Paperclip className="w-4 h-4"/> Investigations</h3>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center"><Label className="text-slate-700">Prescription</Label><Button variant="ghost" size="sm" onClick={addMed} className="text-blue-600 hover:bg-blue-50"><Plus className="w-3 h-3 mr-1"/> Add Drug</Button></div>
                    {medicines.map((med, idx) => (
                        <div key={idx} className="grid grid-cols-12 gap-2 items-center bg-slate-50 p-2 rounded border border-slate-100">
                            ...
                            <div className="col-span-2 flex gap-1"><Input placeholder="Instr" value={med.instructions} onChange={(e)=>handleMedChange(idx,"instructions",e.target.value)} className="h-8 bg-white border-slate-200"/><Button variant="ghost" size="icon" onClick={()=>removeMed(idx)} className="h-8 w-8 text-red-400 hover:text-red-600"><Trash2 className="w-3 h-3"/></Button></div>
                        </div>
                    ))}
                </div>
                <Separator />
    ...
    return (
        <div className="space-y-8 pb-4">
            <div className="flex justify-between bg-slate-50 p-4 rounded-lg border border-slate-100">
                <div><p className="text-xs text-slate-400 uppercase font-bold">Patient</p><p className="font-bold text-slate-800">{data.patientDetails?.name}</p></div>
                <div className="text-right"><p className="text-xs text-slate-400 uppercase font-bold">Date</p><p className="font-semibold text-slate-800">{new Date(data.createdAt).toLocaleDateString()}</p></div>
            </div>

            <div className="grid grid-cols-5 gap-2 text-center">
                {Object.entries(data.vitalSigns || {}).map(([key, val]) => (
                    val && <div key={key} className="border border-slate-100 p-2 rounded bg-white"><p className="text-[10px] text-slate-400 uppercase font-bold">{key}</p><p className="font-bold text-slate-700">{val}</p></div>
                ))}
            </div>
            
            <Separator />
            
            <div><h4 className="font-bold text-slate-900 mb-1">Diagnosis</h4><p className="text-lg text-blue-800">{data.diagnosis}</p><p className="text-sm text-slate-500">{data.symptoms}</p></div>
            
            <Separator />
            
            <div>
                <h4 className="font-bold text-slate-900 mb-3 flex items-center gap-2"><Pill className="w-4 h-4 text-blue-500"/> Medications</h4>
                {data.prescription?.map((med, i) => (
                    <div key={i} className="flex justify-between bg-slate-50 p-2 rounded text-sm border-b border-white mb-1"><span className="font-semibold w-1/3">{med.medicineName}</span><span className="text-slate-600">{med.dosage} | {med.frequency} | {med.duration}</span></div>
                ))}
            </div>
            ...
            {data.labReports?.length > 0 && (
                <div>
                    <h4 className="font-bold text-slate-900 mb-3 flex items-center gap-2"><Paperclip className="w-4 h-4 text-orange-500"/> Reports</h4>
                    <div className="flex flex-wrap gap-2">
                        {data.labReports.map((file, i) => (
                            <Badge key={i} variant="outline" className="pl-3 pr-1 py-1 flex items-center gap-2 cursor-pointer hover:bg-slate-50" onClick={()=>downloadFile(file.url)}>
            ...
            {data.patientAdvice && (
                <div className="bg-blue-50 p-4 rounded-md border border-blue-100 mt-4">
                    <h4 className="text-sm font-bold text-blue-900 mb-1">Advice</h4>
                    <p className="text-sm text-blue-800 whitespace-pre-wrap">{data.patientAdvice}</p>
                </div>
            )}
        </div>
    )
